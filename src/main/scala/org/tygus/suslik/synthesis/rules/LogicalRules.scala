  ...
  object EmpRule extends SynthesisRule with InvertibleRule {
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      if (pre.sigma.isEmp && post.sigma.isEmp && // heaps are empty
        ...
        {
        //println("Result: " + post.sigma.tps.result_var)
        List(RuleResult(Nil, ConstProducer(Return(post.sigma.tps.result_var)), this, goal)) // we are done
        }
      else Nil
    }
  }
  ...
  object SimplifyConditional extends SynthesisRule with InvertibleRule {
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      goal.post.sigma.chunks.find {
        case h@PointsTo(_, _, IfThenElse(_, _, _), _) => h.vars.forall(v => !goal.isGhost(v))
        case _ => false
      } match {
        case None => Nil
        case Some(h@PointsTo(l, o, IfThenElse(c, t, e), p)) =>
          if (SMTSolving.valid(goal.pre.phi ==> (c || (t |=| e)))) {
            val thenSigma = (goal.post.sigma - h) ** PointsTo(l, o, t, p)
            ...
            val elseSigma = (goal.post.sigma - h) ** PointsTo(l, o, e, p)
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      findMatchingHeaplets(_ => true, isMatch, pre.sigma, post.sigma) match {
        ...
        case Some((hPre, hPost)) => {
          ...
          //println("Framing out: " + hPre + " and " + hPost)
          val kont = IdProducer >> ExtractHelper(goal)
  ...
  */
  ...
  object StarPartial extends SynthesisRule with InvertibleRule {
    ...
    def extendPure(p: PFormula, s: SFormula): PFormula = {
      val ptrs = (for (PointsTo(x, o, _, _) <- s.chunks) yield (o, x)).groupBy(_._1).mapValues(_.map(_._2))
  ...
  object GhostWrite extends SynthesisRule with InvertibleRule {
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      def pointsToGhosts: Heaplet => Boolean = {
        case PointsTo(x@Var(_), _, e, _) => !goal.isGhost(x) && e.vars.exists(v => goal.universalGhosts.contains(v))
        case _ => false
      }
      ...
      val firstHeaplet = SFormula(goal.post.sigma.chunks.take(1), TypeMap())
      findMatchingHeaplets(_ => true, isMatch, goal.pre.sigma, firstHeaplet) match {
        ...
        case Some((_, hr@PointsTo(x, offset, e2, p))) =>
          ...
          val newPost = Assertion(
            post.phi && (ex |=| e2),
            (goal.post.sigma - hr) ** PointsTo(x, offset, ex, p))
