  ...
  object SimplifyConditional extends SynthesisRule with InvertibleRule {
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      goal.post.sigma.chunks.find {
        case h@PointsTo(_, _, IfThenElse(_, _, _), _, _) => h.vars.forall(v => !goal.isGhost(v))
        case _ => false
      } match {
        case None => Nil
        case Some(h@PointsTo(l, o, IfThenElse(c, t, e), p, tp)) =>
          if (SMTSolving.valid(goal.pre.phi ==> (c || (t |=| e)))) {
            val thenSigma = (goal.post.sigma - h) ** PointsTo(l, o, t, p, tp)
            ...
            val elseSigma = (goal.post.sigma - h) ** PointsTo(l, o, e, p, tp)
  ...
  */
  ...
  object StarPartial extends SynthesisRule with InvertibleRule {
    ...
    def extendPure(p: PFormula, s: SFormula): PFormula = {
      val ptrs = (for (PointsTo(x, o, _, _, _) <- s.chunks) yield (o, x)).groupBy(_._1).mapValues(_.map(_._2))
  ...
  object GhostWrite extends SynthesisRule with InvertibleRule {
    ...
    def apply(goal: Goal): Seq[RuleResult] = {
      ...
      def pointsToGhosts: Heaplet => Boolean = {
        case PointsTo(x@Var(_), _, e, _, _) => !goal.isGhost(x) && e.vars.exists(v => goal.universalGhosts.contains(v))
        case _ => false
      }
